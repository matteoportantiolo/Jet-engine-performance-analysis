...
%SONO FATTI SULL'ESTREMITA' SU CUI DEFINIAMO UN GRADO DI REAZIONE DI 0.9
%LE MATRICI SCORRONO GLI STADI SULLA COLONNA E TUTTI I POSSIBILI ALPHA SULLA RIGA
%N.B. il vettore ALPHA è stato fatto variare prima da -pi/2 a pi/2 e poi è stato diminuito il passo intorno al 
%valore trovato
ALPHA=-pi/15:pi/100000:0;
%diametri e raggi
d_ext=21.48*0.0254;
d_int_in=d_ext*0.5;
hubtotip_out=0.645;
n_stadi=8;
d_int_out=d_ext*hubtotip_out;
m=((d_int_out-d_int_in)/2)/(n_stadi-1);
% N.B. 
...
BETA_st(4,1)=BETA_stadio*1.005;
BETA_st(5,1)=BETA_stadio/1.005;
BETA_st(3,1)=BETA_stadio*1.015;
BETA_st(2,1)=BETA_stadio*1.025;
BETA_st(1,1)=BETA_stadio*1.035;
BETA_st(6,1)=BETA_stadio/1.015;
BETA_st(7,1)=BETA_stadio/1.025;
BETA_st(8,1)=BETA_stadio/1.035;
...
for j=1:length(ALPHA)
 ...
 T3_tot_reale=393.481656012071;%uscita compressore
 % ricavare eta stadio
 ...
 beta2(i,j)=atan(((-R+0.5)*2*norm([0;U(i)]))/C1_z(i,j)-tan(alpha1(i,j)));
 ...
 rapporto_eulero(i,j)=(1+(om*r_c(end)*(C2_z(i,j)*tan(alpha2(i,j))- ...
     C1_z(i,j)*tan(alpha1(i,j))))/(cp_a*T1_tot(i,j)))^(gamma_a/(gamma_a-1));
...
grado_reazione_r=zeros(length(r_c),n_stadi); 
rapporto_eulero_r=zeros(length(r_c),n_stadi); 
for j=1:n_stadi
 for i=1:length(r_c)
 r=r_c(i);
 U_r(i)=om*r;
...
xlabel('alpha_1 [rad]')
ylabel('P_2_t_o_t/P_1_t_o_t')
title('Jumo 004 estremità primo stadio')
...
plot(rateo, Mach_r_r(:,8),'linewidth',2);
legend('stadio 1','stadio 2','stadio 3','stadio 4','stadio 5','stadio 6','stadio 7','stadio 8')
xlabel('r/r_t_i_p')
ylabel('Ma_r_e_l')
title('Jumo 004')
%% RAFFIGURAZIONI DEI TRIANGOLI DI VELOCITA'
x0 = 0;
y0 = 0;
colore1 = uisetcolor([0.5, 0.3, 0.1]);
colore2 = uisetcolor([0.5, 0.3, 0.1]);
colore3 = uisetcolor([0.5, 0.3, 0.1]);
colore4 = uisetcolor([0.5, 0.3, 0.1]);
colore5 = uisetcolor([0.5, 0.3, 0.1]);
% TIP
figure
axis equal
hold on
% triangolo in entrata al rotore
quiver(x0,y0,c1_z_r(end,1),-c1_th_r(end,1), 'off','LineWidth', 2, 'color', colore1)
quiver(x0,y0,w1_z_r(end,1),-w1_th_r(end,1), 'off', 'LineWidth', 2, 'color', colore2);
quiver(w1_z_r(end,1),-w1_th_r(end,1), 0, -U_r(end), 'off', 'LineWidth', 2, 'color', colore3);
% triangolo in uscita dal rotore
quiver(x0, w2_th_r(end,1)-w1_th_r(end,1), c2_z_r(end,1),-c2_th_r(end,1), 'off', 'LineWidth', 2, 'color', colore4)
quiver(x0, w2_th_r(end,1)-w1_th_r(end,1), w2_z_r(end,1),-w2_th_r(end,1), 'off', 'LineWidth', 2, 'color', colore5)
title ("Triangoli di velocità all'estremità della paletta")
legend ('c1', 'w1', 'U', 'c2', 'w2')
xlabel('Componente assiale [m/s]');
ylabel('Componente tangenziale [m/s]')
% RAGGIO MEDIO
indice_r_medio = ceil(length(U_r)/2);
figure
axis equal
hold on
% triangolo in entrata al rotore
quiver(x0,y0,c1_z_r(indice_r_medio,1),-c1_th_r(indice_r_medio,1), 'off','LineWidth', 2, 'color', colore1)
quiver(x0,y0,w1_z_r(indice_r_medio,1),-w1_th_r(indice_r_medio,1), 'off', 'LineWidth', 2, 'color', colore2);
quiver(w1_z_r(indice_r_medio,1),-w1_th_r(indice_r_medio,1), 0, -U_r(indice_r_medio), 'off', 'LineWidth', 2, ...
    'color', colore3);
% triangolo in uscita dal rotore
quiver(x0, w2_th_r(indice_r_medio,1)-w1_th_r(indice_r_medio,1), c2_z_r(indice_r_medio,1),- ...
    c2_th_r(indice_r_medio,1), 'off', 'LineWidth', 2, 'color', colore4)
quiver(x0, w2_th_r(indice_r_medio,1)-w1_th_r(indice_r_medio,1), w2_z_r(indice_r_medio,1),- ...
    w2_th_r(indice_r_medio,1), 'off', 'LineWidth', 2, 'color', colore5);
title ("Triangoli di velocità al raggio medio della paletta")
legend ('c1', 'w1', 'U', 'c2', 'w2')
xlabel('Componente assiale [m/s]');
ylabel('Componente tangenziale [m/s]')
% HUB
...
hold on
% triangolo in entrata al rotore
quiver(x0,y0,c1_z_r(1,1),-c1_th_r(1,1), 'off','LineWidth', 2, 'color', colore1)
quiver(x0,y0,w1_z_r(1,1),-w1_th_r(1,1), 'off', 'LineWidth', 2, 'color', colore2);
quiver(w1_z_r(1,1),-w1_th_r(1,1), 0, -U_r(1), 'off', 'LineWidth', 2, 'color', colore3);
% triangolo in uscita dal rotore
quiver(x0, w2_th_r(1,1)-w1_th_r(1,1), c2_z_r(1,1),-c2_th_r(1,1), 'off', 'LineWidth', 2, 'color', colore4)
quiver(x0, w2_th_r(1,1)-w1_th_r(1,1), w2_z_r(1,1),-w2_th_r(1,1), 'off', 'LineWidth', 2, 'color', colore5);
title ("Triangoli di velocità alla radice della paletta")
legend ('c1', 'w1', 'U', 'c2', 'w2')
xlabel('Componente assiale [m/s]');
ylabel('Componente tangenziale [m/s]')
