...
n_stadi=7;
ALPHA=pi/8:pi/1000:pi/5;
%diametri e raggi
d_c_ext=21.65*0.0254;
d_c_int=d_c_ext*0.6;
d_c_int_fin=d_c_ext*0.725;
m=(d_c_int_fin/2-d_c_int/2)/(n_stadi-1);
...
for j=1:length(ALPHA)
    ...
    T3_tot_vera=3.772198236225885e+02; %N.B.: data la velocità e la quota a cui scegliamo di andare, vedere ciclo camo
    eta_c=0.78;
...
c1_z_r=zeros(length(r_c),n_stadi);
...
quiver ([0,0],[0,0],[c1_th_r(end,1),c1_z_r(end,1)],[w1_th_r(end,1),w1_z_r(end,1)],"off")