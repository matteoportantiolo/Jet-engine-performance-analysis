...
for j=1:length(ALPHA)
    ...
    for i=1:n_stadi
        ...
        rapporto_eulero(i,j)=(1+(om*r_c(end)*(C2_z(i,j)*tan(alpha2(i,j))-C1_z(i,j)*tan(alpha1(i,j))))/(cp_a*T1_tot(i,j)))^(gamma_a/(gamma_a-1));
...
for j=1:n_stadi
    for i=1:length(r_c)
        ...
        rapporto_eulero_r(i,j)=(1+(om*r*(c2z*tan(alpha2_r(i,j))-c1z*tan(alpha1_r(i,j))))/(cp_a*T1_tot(j,1)))^(gamma_a/(gamma_a-1));
    end
end
%% 
