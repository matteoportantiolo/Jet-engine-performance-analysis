...
%diametri e raggi
d_c_ext=21.65*0.0254;
d_c_int=d_c_ext*0.6;
n_stadi=7;
hubtotip_f=0.725;
d_c_int_f=d_c_ext*hubtotip_f;
m=((d_c_int_f-d_c_int)/2)/(n_stadi-1);
...
perdita = 0.98;%^(1/7);%
   ...
   r_c=d_c_int/2:m:d_c_int_f/2;
    for k=1:n_stadi
        r_int=d_c_int/2+(k-1)*m;
        r_med=(r_int+(d_c_ext/2))/2;
        r_c=[r_c, r_med];
    end
    r_c=[r_c, d_c_ext/2];
...
for j=1:length(ALPHA)
    ...
    T3_tot_vera=377.219823622588; %N.B.: data la velocità e la quota a cui scegliamo di andare, vedere ciclo camo
    eta_c=0.78;
    ...
    for i=1:n_stadi
        ...
        rapporto_rotore(i,j)=(1+(om*r_m*(C2_z(i,j)*tan(alpha2(i,j))-C1_z(i,j)*tan(alpha1(i,j))))/(cp_a*T1_tot(i,j)))^(gamma_a/(gamma_a-1));%eulero
...
c2_z_r=155*ones(length(r_c),n_stadi);
...
quiver(w1_z_r(indice_r_medio,1),-w1_th_r(indice_r_medio,1), 0, -U_r(indice_r_medio), 'off', 'LineWidth', 2, 'color', colore3);
% triangolo in uscita dal rotore
quiver(x0, w2_th_r(indice_r_medio,1)-w1_th_r(indice_r_medio,1), c2_z_r(indice_r_medio,1),-c2_th_r(indice_r_medio,1), 'off', 'LineWidth', 2, 'color', colore4)
quiver(x0, w2_th_r(indice_r_medio,1)-w1_th_r(indice_r_medio,1), w2_z_r(indice_r_medio,1),-w2_th_r(indice_r_medio,1), 'off', 'LineWidth', 2, 'color', colore5);
